...
static inline void copy_bytes(char **str, char *dest)
{
...
int config_load(const char *configpath)
{
    ...
    while (fgets(line, 0xff, fh) != NULL)
    {
        ...
        if (line[0] == '\0')
        {
            p_warning("WARNING: Incomplete configuration '%s' at line %d. "
                        "Fallback to default.",
                        key, linenr);
            continue;
        }
...
void config_print(void)
{
    if (config.loglevel < WARNING)
    {
        ...
        {
            p_info("\tUnix socket: %s", config.hostname);
        }
        else
        {
            p_info("\tAddress: %s", config.hostname);
            p_info("\tPort: %s", config.port);
            p_info("\tTcp backlog: %d", config.tcp_backlog);
        }
        const char *human_rsize = memory_to_string(config.max_request_size);
        p_info("\tMax request size: %s", human_rsize);
        p_info("Logging:");
        p_info("\tlevel: %s", llevel);
        p_info("\tlogpath: %s", config.logpath);
        const char *human_memory = memory_to_string(config.max_memory);
        p_info("Max memory: %s", human_memory);
        p_free((char *)human_memory);
        p_free((char *)human_rsize);
    }
}
