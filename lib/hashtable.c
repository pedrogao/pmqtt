...
static int hashtable_rehash(HashTable *table)
{
    ...
    struct hashtable_entry *temp =
        p_calloc(2 * table->table_size, sizeof(*temp));
...
HashTable *hashtable_create(int (*destructor)(struct hashtable_entry *))
{
    ...
    HashTable *table = p_malloc(sizeof(HashTable));
    if (!table)
        return NULL;

    table->entries = p_calloc(INITIAL_SIZE, sizeof(struct hashtable_entry));
    if (!table->entries)
    {
