                                  ...
                                  union mqtt_packet *pkt)
{
    ...
    if (cid_len > 0)
    {
        pkt->connect.payload.client_id = p_malloc(cid_len + 1);
    ...
    if (pkt->connect.bits.will == 1)
    {
        ...
        pkt->connect.payload.will_topic = p_malloc(will_topic_len + 1);
        ...
        pkt->connect.payload.will_message = p_malloc(will_message_len + 1);
    ...
    if (pkt->connect.bits.username == 1)
    {
        ...
        pkt->connect.payload.username = p_malloc(username_len + 1);
    ...
    if (pkt->connect.bits.password == 1)
    {
        ...
        pkt->connect.payload.password = p_malloc(password_len + 1);
                                  ...
                                  union mqtt_packet *pkt)
{
    ...
    pkt->publish.topic = p_malloc(topic_len + 1);
    ...
    pkt->publish.payload = p_malloc(message_len + 1);
                                    ...
                                    union mqtt_packet *pkt)
{
    ...
    while (remaining_bytes > 0)
    {
        ...
        subscribe.tuples = p_realloc(subscribe.tuples,
                                       (i + 1) * sizeof(*subscribe.tuples));
        subscribe.tuples[i].topic_len = topic_len;
        subscribe.tuples[i].topic = p_malloc(topic_len + 1);
                                      ...
                                      union mqtt_packet *pkt)
{
    ...
    while (remaining_bytes > 0)
    {
        ...
        /* We have to make room for additional incoming tuples */
        unsubscribe.tuples = p_realloc(unsubscribe.tuples,
                                         (i + 1) * sizeof(*unsubscribe.tuples));
        unsubscribe.tuples[i].topic_len = topic_len;
        unsubscribe.tuples[i].topic = p_malloc(topic_len + 1);
        unpack_bytes((const uint8_t **)&raw, topic_len,
...
static unsigned char *pack_mqtt_header(const union mqtt_header *hdr)
{
    ...
    unsigned char *packed = p_malloc(MQTT_HEADER_LEN);
...
static unsigned char *pack_mqtt_ack(const union mqtt_packet *pkt)
{
    ...
    unsigned char *packed = p_malloc(MQTT_ACK_LEN);
    unsigned char *ptr = packed;
...
static unsigned char *pack_mqtt_suback(const union mqtt_packet *pkt)
{
    ...
    unsigned char *packed = p_malloc(pktlen + 0);
    unsigned char *ptr = packed;
...
static unsigned char *pack_mqtt_publish(const union mqtt_packet *pkt)
{
    ...
    unsigned char *packed = p_malloc(pktlen);
    unsigned char *ptr = packed;
                                       ...
                                       unsigned short rcslen)
{
    ...
    struct mqtt_suback *suback = p_malloc(sizeof(*suback));

    suback->header.byte = byte;
    suback->pkt_id = pkt_id;
    suback->rcslen = rcslen;
    suback->rcs = p_malloc(rcslen);
                                         ...
                                         unsigned char *payload)
{
    ...
    struct mqtt_publish *publish = p_malloc(sizeof(*publish));
...
void mqtt_packet_release(union mqtt_packet *pkt, unsigned type)
{
    ...
    {
    case CONNECT:
        p_free(pkt->connect.payload.client_id);
        if (pkt->connect.bits.username == 1)
            p_free(pkt->connect.payload.username);
        if (pkt->connect.bits.password == 1)
            p_free(pkt->connect.payload.password);
        if (pkt->connect.bits.will == 1)
        {
            p_free(pkt->connect.payload.will_message);
            p_free(pkt->connect.payload.will_topic);
        }
        break;
    case SUBSCRIBE:
    case UNSUBSCRIBE:
        for (unsigned i = 0; i < pkt->subscribe.tuples_len; i++)
            p_free(pkt->subscribe.tuples[i].topic);
        p_free(pkt->subscribe.tuples);
        break;
    case SUBACK:
        p_free(pkt->suback.rcs);
        break;
    case PUBLISH:
        p_free(pkt->publish.topic);
        p_free(pkt->publish.payload);
        break;
    default:
        break;
    }
