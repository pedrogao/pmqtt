...
{
    buf->bytes = p_malloc(INITIAL_BUFLEN);
...
void buffer_push_back(struct buffer *buf, unsigned char *bytes, size_t len)
{
    ...
    {
        buf->size *= 2;
        buf->bytes = p_realloc(buf->bytes, buf->size);
    }
...
struct evloop *evloop_create(int max_events, int timeout)
{
    ...
    struct evloop *loop = p_malloc(sizeof(*loop));
...
void evloop_init(struct evloop *loop, int max_events, int timeout)
{
    ...
    loop->events = p_malloc(sizeof(struct epoll_event) * max_events);
    loop->epollfd = epoll_create1(0);
    loop->timeout = timeout;
    loop->periodic_maxsize = EVLOOP_INITIAL_SIZE;
    loop->periodic_nr = 0;
    loop->periodic_tasks =
        p_malloc(EVLOOP_INITIAL_SIZE * sizeof(*loop->periodic_tasks));
    loop->status = 0;
}
...
void evloop_free(struct evloop *loop)
{
    ...
    for (int i = 0; i < loop->periodic_nr; i++)
        p_free(loop->periodic_tasks[i]);
    p_free(loop->periodic_tasks);
    p_free(loop);
}
