...
static int connect_handler(struct closure *cb, union mqtt_packet *pkt)
{
     ...
     */
    struct p_client *new_client = p_malloc(sizeof(*new_client));
    new_client->fd = cb->fd;
    ...
    union mqtt_packet *response = p_malloc(sizeof(*response));
...
static int subscribe_handler(struct closure *cb, union mqtt_packet *pkt)
{
    ...
    for (unsigned i = 0; i < pkt->subscribe.tuples_len; i++)
    {
        ...
        p_debug("Received SUBSCRIBE from %s", c->client_id);
        ...
        p_debug("\t%s (QoS %i)", topic, pkt->subscribe.tuples[i].qos);
    ...
    p_debug("Sending SUBACK to %s", c->client_id);
...
static int unsubscribe_handler(struct closure *cb, union mqtt_packet *pkt)
{
    ...
    p_debug("Received UNSUBSCRIBE from %s", c->client_id);
    ...
    p_debug("Sending UNSUBACK to %s", c->client_id);
...
static int publish_handler(struct closure *cb, union mqtt_packet *pkt)
{
    ...
    if (qos == AT_LEAST_ONCE)
    {
        ...
        p_debug("Sending PUBACK to %s", c->client_id);
    ...
    else if (qos == EXACTLY_ONCE)
    {
        ...
        p_debug("Sending PUBREC to %s", c->client_id);
...
static int pubrec_handler(struct closure *cb, union mqtt_packet *pkt)
{
    ...
    p_debug("Received PUBREC from %s", c->client_id);
    ...
    p_debug("Sending PUBREL to %s", c->client_id);
...
static void on_read(struct evloop *loop, void *arg)
{
    ...
    unsigned char *buffer = p_malloc(conf->max_request_size);
...
static void on_accept(struct evloop *loop, void *arg)
{
    ...
    struct closure *client_closure = p_malloc(sizeof(*client_closure));
    if (!client_closure)
        return;
    ...
    p_info("New connection from %s on port %s", conn.ip, conf->port);
}
...
static void run(struct evloop *loop)
{
    ...
    {
        p_error("Event loop exited unexpectedly: %s", strerror(loop->status));
        evloop_free(loop);
    }
}
...
static void publish_stats(struct evloop *loop, void *args)
{
    ...
    double p_uptime = (double)(time(NULL) - info.start_time) / p_SECONDS;
    char sutime[16];
    sprintf(sutime, "%.4f", p_uptime);
...
static int client_destructor(struct hashtable_entry *entry)
{
    ...
    struct p_client *client = entry->val;

    if (client->client_id)
        p_free(client->client_id);
